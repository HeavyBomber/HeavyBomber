...
namespace SpriteSheetPipeline
{
  ...
  /// </summary>
  [ContentProcessor]
  public class SpriteSheetProcessor : ContentProcessor<Sprite, SpriteSheetContent>
  {
    /// <summary>
    /// Converts an array of sprite filenames into a sprite sheet object.
    /// </summary>
    public override SpriteSheetContent Process(Sprite input,
                                               ContentProcessorContext context)
    {
      SpriteSheetContent spriteSheet = new SpriteSheetContent();
      List<BitmapContent> sourceSprites = new List<BitmapContent>();

        spriteSheet.FrameTime = input.FrameTime;
        spriteSheet.FramesPerDir = input.FramesPerDir;
      // Loop over each input sprite filename.
      foreach (string inputFilename in input.SourceSprites)
      {
